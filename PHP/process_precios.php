...
if (isset($_GET['consultar'])) {
    $orden = $_GET['orden'];
    $sql = "SELECT descripcion, precio, fecha_creacion, imagen FROM precios";
...
/*/////////////////////////////////     Edicion de video de fondo    ////////////////////////////////////////*/
    ...
    echo ' <form action="process_precios.php" method="get" class="adj">
            ...
            <select name="orden" id="orden" class="boton">
                <option value="precio_asc" ' . (isset($orden) && $orden == 'precio_asc' ? 'selected' : '') . '>De menor a mayor</option>
                <option value="precio_desc" ' . (isset($orden) && $orden == 'precio_desc' ? 'selected' : '') . '>De mayor a menor</option>
                <option value="fecha_asc" ' . (isset($orden) && $orden == 'fecha_asc' ? 'selected' : '') . '>Antiguos</option>
                <option value="fecha_desc" ' . (isset($orden) && $orden == 'fecha_desc' ? 'selected' : '') . '>Nuevos</option>
            </select>
            <input class="bt" type="submit" name="consultar" value="Buscar">
        </form>';

    if ($result->num_rows > 0) {
        echo "<table><tr><th>Imagen</th><th>Descripción</th><th>Precio</th><th>Fecha de Creación</th></tr>";
    ...
    echo'<center>
            <footer>
                ...
                | <strong class="fot">Perfil de </strong><a class="nad" href="politica_privacidad.html">Github</a> </p>
            </footer>
        </center>';
}
